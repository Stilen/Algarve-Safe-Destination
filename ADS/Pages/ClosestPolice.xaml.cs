    ...
    public sealed partial class ClosestPolice : Page
    {
        ...
        private async void CalcDist()
        {
            ...
            try
            {
                ...
                CurrentP.Latitude = position.Coordinate.Point.Position.Latitude;
                CurrentP.Longitude = position.Coordinate.Point.Position.Longitude;
        ...
        private async void Destinationbox_Tapped(object sender, TappedRoutedEventArgs e)
        {
            //this.closest.dest = true;
            //Frame.Navigate(typeof(Pages.Contactos), closest); 

            DatabaseHelperClass dbtemas = new DatabaseHelperClass();
            string latitude = dbtemas.latitude(closest.N_Vertice).ToString();
            string longitude = dbtemas.longitude(closest.N_Vertice).ToString();

            Geolocator geolocator = new Geolocator();
            geolocator.DesiredAccuracyInMeters = 5;
            Geoposition position = await geolocator.GetGeopositionAsync();

            Geocoordinate myPosition = position.Coordinate;
            string myLatitude = myPosition.Point.Position.Latitude.ToString();
            string myLongitude = myPosition.Point.Position.Longitude.ToString();

            Uri uri = new Uri("bingmaps:?rtp=pos." + myLatitude + "_" + myLongitude +
                "~pos." + latitude + "_" + longitude);
